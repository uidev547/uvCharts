cv.bargraph = function (graphdef) {
	...
	for(var i=0, len=this.dataset.length; i<len; i++){
		...
		if(this.graphdef.orientation === 'hor') {
			this.drawHorBars(bars, len); bargroup.attr("transform","translate(0," + i*this.axes.ver.scale.rangeBand()/len + ")");
		} else {
			this.drawVerBars(bars, len); bargroup.attr("transform","translate(" + i*this.axes.hor.scale.rangeBand()/len + ",0)");
		}
